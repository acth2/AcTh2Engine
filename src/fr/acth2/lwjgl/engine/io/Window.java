...
public class Window
{
    ...
    public void setFullscreen(boolean isFullscreen){
        ...
        if (isFullscreen){
            GLFW.glfwSetWindowSize(window, screenWidth, screenHeight);
            GLFW.glfwSetWindowPos(window, 0, 0);
        }else  {
            GLFW.glfwSetWindowSize(window, WIDTH[0], HEIGHT[0]);
            GLFW.glfwSetWindowPos(window, xPos,  yPos);
            if (Main.os.contains("win")) { try { Runtime.getRuntime().exec("explorer.exe"); } catch (IOException e) { throw new RuntimeException(e); } }
            Main.fullscreenAtomic.set(false);
        }
    }
